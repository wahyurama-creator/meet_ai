...
import { CallRecordingReadyEvent, CallSessionEndedEvent, CallSessionParticipantLeftEvent, CallSessionStartedEvent, CallTranscriptionReadyEvent } from "@stream-io/node-sdk";
    ...
    if (eventType === "call.session_started") {
        ...
        const [existingMeeting] = await db
            ...
            .where(
                and(
                    ...
                    not(eq(meetings.meetingStatus, "completed")),
                    not(eq(meetings.meetingStatus, "cancelled")),
                    not(eq(meetings.meetingStatus, "active")),
                    not(eq(meetings.meetingStatus, "processing")),
                )
            )
        ...
        await db
            ...
            .where(
                and(
                    eq(meetings.id, meetingId),
                    eq(meetings.meetingStatus, "active"),
                )
            )
